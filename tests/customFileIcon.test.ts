...
import { CustomFileIcon } from '../src/components/CustomFileIcon'
...
describe('Custom File Category & Pastel Theme Palette', () => {
  ...
  it('renders every large pastel category as standalone SVG markup', () => {
    const extensions = ['pdf', 'docx', 'xlsx', 'pptx', 'zip', 'txt', 'ts', 'mp3', 'mp4', 'png', 'exe', 'unknown']

    for (const ext of extensions) {
      const markup = renderToStaticMarkup(createElement(CustomFileIcon, { ext, width: 32, height: 32 }))
      expect(markup).toContain('<svg')
      expect(markup).toContain('viewBox="0 0 512 512"')
    }

    const folderMarkup = renderToStaticMarkup(createElement(CustomFileIcon, { path: 'C:\\Projects', isDirectory: true, width: 32 }))
    expect(folderMarkup).toContain('fill="#FBBF24"')
  })

  it('keeps compact badges on the simplified micro icon path', () => {
    const markup = renderToStaticMarkup(createElement(CustomFileIcon, { kind: 'word', width: 12 }))
    expect(markup).toContain('viewBox="0 0 24 24"')
    expect(markup).toContain('#7BAFF8')
  })
})
